...
└─────────────────────────────────────────────────────────────────┘
                              │
                              │ 1:1 (optional - only for businesses)
                              │
                              ▼
            ┌──────────────────────────────────┐
            │ BUSINESS_ORGANIZATIONS (MINIMAL) │
            │ • proceeds_percentage            │
...
└────────────────────────────────────────────────────────┘
                                                        │
Note: proceeds_description, impact_statement,         │
      ships_nationally, women_owned → in description  │
      or tags, NOT fields                             │
                    │                                   │
                    │ has listings                      │
                    ▼                                   │
...
Note: NO nonprofit_organizations table.
      Everything in base organizations table + tags.
```
...
```
organizations
├── organization_type = 'business' → business_organizations (MINIMAL: 6 fields)
├── organization_type = 'nonprofit' → no extension table (use description + tags)
├── organization_type = 'community' → no extension table (use description + tags)
