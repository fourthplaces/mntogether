...
const logGroup = new aws.cloudwatch.LogGroup("web-next-logs", {
  name: `/mndigitalaid/${config.stack}/web-next`,
...
const albSecurityGroup = new aws.ec2.SecurityGroup("web-next-alb-sg", {
  vpcId,
  ingress: [
    {
      protocol: "tcp",
  ...
  tags: getStandardTags({ Name: "web-next-alb-sg" }),
});

const serviceSecurityGroup = new aws.ec2.SecurityGroup("web-next-service-sg", {
  vpcId,
  ingress: [
    {
      protocol: "tcp",
  ...
  tags: getStandardTags({ Name: "web-next-service-sg" }),
});

// ========== Application Load Balancer ==========

const alb = new aws.lb.LoadBalancer("web-next-alb", {
  internal: false,
  loadBalancerType: "application",
  securityGroups: [albSecurityGroup.id],
  subnets: subnetIds,
  enableDeletionProtection: config.isProd,
  tags: getStandardTags({ Name: "web-next-alb" }),
});

// Target group for ECS service
const targetGroup = new aws.lb.TargetGroup("web-next-tg", {
  port: 3000,
  protocol: "HTTP",
  vpcId,
  targetType: "ip",
...
// HTTPS listener
new aws.lb.Listener("web-next-https", {
...
new aws.lb.Listener("web-next-http", {
  loadBalancerArn: alb.arn,
  port: 80,
  protocol: "HTTP",
  defaultActions: [
...
const cluster = new aws.ecs.Cluster("web-next-cluster", {
  name: `mndigitalaid-web-next-${config.stack}`,
  settings: [
    {
...
// Task execution role
const taskExecutionRole = new aws.iam.Role("web-next-task-execution-role", {
...
new aws.iam.RolePolicyAttachment("web-next-task-execution-role-policy", {
  role: taskExecutionRole.name,
  policyArn: "arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy",
});

// Task role
const taskRole = new aws.iam.Role("web-next-task-role", {
  assumeRolePolicy: JSON.stringify({
    Version: "2012-10-17",
    Statement: [
      {
...
// Task definition
const taskDefinition = new aws.ecs.TaskDefinition("web-next-task", {
  family: `mndigitalaid-web-next-${config.stack}`,
  networkMode: "awsvpc",
    ...
    JSON.stringify([
      {
        ...
        logConfiguration: {
          ...
          options: {
            ...
            "awslogs-stream-prefix": "web-next",
          },
        },
      },
    ])
...
// ECS Service
const service = new aws.ecs.Service("web-next-service", {
  cluster: cluster.arn,
