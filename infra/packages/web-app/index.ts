...
const webAppDir = path.join(packagesDir, "web-app", "dist");
...
const siteBucket = new aws.s3.Bucket(
  `${subdomain}-${domainParts.parentDomain}`,
  {
    ...
    tags: getStandardTags({ Name: "web-app-bucket" }),
  }
);
...
uploadStaticAssets(webAppDir, siteBucket);
...
const distribution = new aws.cloudfront.Distribution("app-cdn", {
  ...
  tags: getStandardTags({ Name: "app-cdn" }),
});

// Grant CloudFront access to S3 bucket
new aws.s3.BucketPolicy("app-bucket-policy", {
