...
-- lead-alert-digest: alert (h=5) is shorter than gazette (h=8), so only
-- 2 digests fit the sidebar height. The unclamped-anchor fix helps
-- pair-stack-gazette (gazette anchor) but not lead-alert-digest (alert anchor).
UPDATE row_template_slots SET count = 2
