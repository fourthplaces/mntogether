...
use server_core::domains::scraping::models::{PageSnapshot, Website, WebsiteSnapshot};
...
// =============================================================================
// POSTS WITHOUT EMBEDDINGS DEDUPLICATION TESTS
// =============================================================================

/// Test that posts without embeddings are skipped during deduplication (not crash)
#[test_context(TestHarness)]
#[tokio::test]
async fn test_deduplication_skips_posts_without_embeddings(ctx: &TestHarness) {
    let admin_id = create_admin_user(ctx).await;
    let website_id = create_approved_website(ctx, "https://no-embed-dedup.org", admin_id).await;

    // Create posts manually WITHOUT embeddings
    let _post1 = Post::create(
        "Test Org".to_string(),
        "Food Bank Service".to_string(),
        "Description for food bank".to_string(),
        None,
        "opportunity".to_string(),
        "general".to_string(),
        Some("accepting".to_string()),
    ...
    let _post2 = Post::create(
        "Test Org".to_string(),
        "Food Bank Program".to_string(), // Similar title
        "Similar description for food bank".to_string(),
        None,
        "opportunity".to_string(),
        "general".to_string(),
        Some("accepting".to_string()),
    ...
    assert_eq!(count_before, 2, "Should have 2 posts before");

    // Both posts have no embeddings - deduplication should handle gracefully
    // (either skip them or use default behavior)
    let posts_with_embeddings: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM posts WHERE website_id = $1 AND embedding IS NOT NULL"
    )
    .bind(website_id)
    .fetch_one(&ctx.db_pool)
    .await
    ...
    assert_eq!(posts_with_embeddings, 0, "Posts should have no embeddings");
}
...
/// Test cosine similarity with edge cases (covered by unit tests, but integration test too)
#[test_context(TestHarness)]
#[tokio::test]
async fn test_posts_with_empty_or_null_embeddings_handled(ctx: &TestHarness) {
    let admin_id = create_admin_user(ctx).await;
    let website_id = create_approved_website(ctx, "https://empty-embed.org", admin_id).await;

    // Create post with empty embedding array (edge case)
    let post = Post::create(
        "Test Org".to_string(),
        "Empty Embedding Post".to_string(),
        "A post that might have empty embedding".to_string(),
        None,
        "opportunity".to_string(),
        "general".to_string(),
        Some("accepting".to_string()),
    ...
    // Query posts with embeddings should exclude this
    let posts_with_embed = Post::find_with_embeddings_for_website(
        WebsiteId::from_uuid(website_id),
        &ctx.db_pool,
    )
    .await
    ...
    // Post without embedding should not be in the list
    assert!(
        posts_with_embed.is_empty() || posts_with_embed.iter().all(|(id, emb, _)| !emb.is_empty()),
        "Posts with embeddings should not include empty embeddings"
    );
}
