...
impl MatchCandidate {
    ...
    /// Find members statewide (no location filter)
    ///
    /// Fallback used when organization has no location data
    #[instrument(skip(need_embedding, pool))]
    pub async fn find_statewide(need_embedding: &[f32], pool: &PgPool) -> Result<Vec<Self>> {
