...
use crate::domains::posts::models::{Post, PostContact, PostStatus, PostWebsiteSync};
...
pub struct SyncResult {
    pub new_listings: Vec<PostId>,
    pub unchanged_listings: Vec<PostId>,
    pub changed_listings: Vec<PostId>,
    pub disappeared_listings: Vec<PostId>,
}
...
pub async fn sync_listings(
    ...
    extracted_listings: Vec<ExtractedPostInput>,
) -> Result<SyncResult> {
    tracing::info!(
    ...
    for listing in extracted_listings {
        ...
        if let Some(sync_record) = existing_sync {
            ...
            unchanged_listings.push(sync_record.get_post_id());
        } else {
            // Check if listing exists by website + title (in case hash changed due to description change)
            let existing_by_title =
                Post::find_by_domain_and_title(website_id, &listing.title, pool).await?;
            ...
            if let Some(existing) = existing_by_title {
                ...
                let urgency = normalize_urgency(listing.urgency.clone());

                match Post::create(
                    listing.organization_name.clone(),
                    listing.title.clone(),
    ...
    let disappeared_uuids =
        PostWebsiteSync::mark_disappeared_except(website_id, seen_hashes, pool).await?;

    let disappeared_listings: Vec<PostId> = disappeared_uuids
        .into_iter()
        .map(PostId::from_uuid)
        .collect();
