...
use crate::common::{ContactInfo as CommonContactInfo, ExtractedPost, WebsiteId};
use crate::domains::crawling::models::{PageExtraction, PageSnapshotId};
use crate::domains::posts::models::Post;
use crate::kernel::{BaseAI, BaseSearchService};
...
pub struct EnrichmentContext<'a> {
    ...
    pub search_service: Option<&'a dyn BaseSearchService>,
    pub ai: &'a dyn BaseAI,
}
                ...
                match name {
                    ...
                    "find_schedule" => {
                        enriched.schedule = Some(ScheduleInfo {
                            hours: args.get("hours").and_then(|v| v.as_str()).map(String::from),
                    ...
                    "search_external" => {
                        ...
                        let result = if let Some(search) = ctx.search_service {
                            match search.search(query, Some(3), Some("basic"), None).await {
                                Ok(results) => {
...
// =============================================================================
// Rig-based Enrichment (preferred)
// =============================================================================

const RIG_ENRICHMENT_PREAMBLE: &str = r#"You are enriching a community resource post with detailed information.
...
/// Enrich a post using rig's agent with proper tool calling
pub async fn enrich_post_with_rig(
    candidate: &PostCandidate,
    page_content: &str,
    page_url: &str,
    page_snapshot_id: Option<Uuid>,
    ...
    let agent = client
        .agent(openai::GPT_4O)
        .preamble(RIG_ENRICHMENT_PREAMBLE)
        .max_tokens(2048)
        .tool(FindContactInfoTool::new(enrichment_data.clone()))
        .tool(FindLocationTool::new(enrichment_data.clone()))
        .tool(FindScheduleTool::new(enrichment_data.clone()))
        .tool(FindEligibilityTool::new(enrichment_data.clone()))
        .tool(FindCallToActionTool::new(enrichment_data.clone()))
        .tool(SearchOtherPagesTool::new(other_pages))
        .tool(FinalizePostTool::new(enrichment_data.clone()))
        .build();
...
When done, call finalize_post with a detailed description and confidence score."#,
    ...
    // Run agent - rig handles tool calling automatically
    let _response = agent
        .prompt(&prompt)
        .await
        .context("Agent enrichment failed")?;

    // Extract collected data
    let data = enrichment_data.read().await;

    let enriched = EnrichedPost {
        title: candidate.title.clone(),
        post_type: candidate.post_type.clone(),
        description: data
            .description
            .clone()
            .unwrap_or_else(|| candidate.brief_description.clone()),
        contact: data.contact.as_ref().map(|c| ContactInfo {
            phone: c.phone.clone(),
            email: c.email.clone(),
            intake_form_url: c.intake_form_url.clone(),
            contact_name: c.contact_name.clone(),
        }),
        location: data.location.as_ref().map(|l| LocationInfo {
            address: l.address.clone(),
            city: l.city.clone(),
            service_area: l.service_area.clone(),
            is_virtual: l.is_virtual,
        }),
        schedule: data.schedule.as_ref().map(|s| ScheduleInfo {
            hours: s.hours.clone(),
            dates: s.dates.clone(),
            frequency: s.frequency.clone(),
            duration: s.duration.clone(),
        }),
        eligibility: data.eligibility.as_ref().map(|e| EligibilityInfo {
            who_qualifies: e.who_qualifies.clone(),
            requirements: e.requirements.clone(),
            restrictions: e.restrictions.clone(),
        }),
        call_to_action: data.call_to_action.as_ref().map(|c| CallToAction {
...
pub async fn extract_from_page(
    ...
    page_snapshot_id: Option<Uuid>,
    other_pages: &HashMap<String, String>,
    search_service: Option<&dyn BaseSearchService>,
    ai: &dyn BaseAI,
...
pub async fn create_post_from_enriched(
    ...
    if let Some(schedule) = &enriched.schedule {
        ...
        if let Some(hours) = &schedule.hours {
...
impl EnrichedPost {
    ...
    pub fn to_extracted_post(&self) -> ExtractedPost {
        ...
        let contact = self.contact.as_ref().map(|c| CommonContactInfo {
            phone: c.phone.clone(),
            email: c.email.clone(),
            website: c.intake_form_url.clone(),
        });
