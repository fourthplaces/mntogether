...
use crate::common::entity_ids::{PostId, WebsiteId};

/// PostWebsiteSync - tracks when listings are seen on websites
///
...
pub struct PostWebsiteSync {
    pub id: Uuid,
    pub post_id: Uuid, // Raw UUID for sqlx compatibility
...
impl PostWebsiteSync {
    ...
    pub async fn find_by_id(id: Uuid, pool: &PgPool) -> Result<Option<Self>> {
        sqlx::query_as::<_, Self>("SELECT * FROM post_website_sync WHERE id = $1")
            .bind(id)
            .fetch_optional(pool)
            .await
            .map_err(Into::into)
    ...
    pub async fn find_by_listing_and_website(
        ...
        sqlx::query_as::<_, Self>(
            "SELECT * FROM post_website_sync WHERE post_id = $1 AND website_id = $2",
        )
        .bind(listing_uuid)
        .bind(website_uuid)
        .fetch_optional(pool)
    ...
    pub async fn find_all_by_listing(post_id: PostId, pool: &PgPool) -> Result<Vec<Self>> {
        let listing_uuid = post_id.into_uuid();
    ...
    pub async fn upsert(
        ...
        sqlx::query_as::<_, Self>(
            ...
            ON CONFLICT (post_id, website_id) DO UPDATE
            SET
    ...
    pub async fn count_active_by_website(website_id: WebsiteId, pool: &PgPool) -> Result<i64> {
        ...
        let count = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM post_website_sync
             WHERE website_id = $1 AND disappeared_at IS NULL",
        )
        .bind(website_uuid)
        .fetch_one(pool)
