...
/// ListingContact - contact information for a listing
#[derive(Debug, Clone, Serialize, Deserialize, sqlx::FromRow)]
pub struct ListingContact {
    pub id: Uuid,
    pub listing_id: Uuid,
...
impl ListingContact {
    ...
    pub async fn create(
        ...
        sqlx::query_as::<_, Self>(
            r#"
            INSERT INTO listing_contacts (listing_id, contact_type, contact_value, contact_label, display_order)
    ...
    pub async fn delete_all_for_listing(listing_id: ListingId, pool: &PgPool) -> Result<u64> {
        let listing_uuid = listing_id.into_uuid();

        let result = sqlx::query("DELETE FROM listing_contacts WHERE listing_id = $1")
            .bind(listing_uuid)
            .execute(pool)
            .await?;
