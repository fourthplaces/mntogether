...
impl From<Listing> for ListingData {
    fn from(listing: Listing) -> Self {
        Self {
            ...
            capacity_status: listing.capacity_status,
            urgency: listing.urgency,
            status: listing.status,
            verified_at: listing.verified_at.map(|dt| dt.to_rfc3339()),
...
impl ListingData {
    ...
    async fn tags(&self, context: &GraphQLContext) -> juniper::FieldResult<Vec<TagData>> {
        let listing_id = ListingId::parse(&self.id)?;
