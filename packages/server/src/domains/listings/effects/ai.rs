...
#[async_trait]
impl Effect<ListingCommand, ServerDeps> for AIEffect {
    ...
    async fn execute(
        ...
        match cmd {
            ListingCommand::ExtractListings {
                source_id,
                job_id,
                organization_name,
                content,
...
async fn handle_extract_listings(
    ...
    let source = match Domain::find_by_id(source_id, &ctx.deps().db_pool).await {
        ...
        Err(e) => {
            ...
            return Ok(ListingEvent::ExtractFailed {
                source_id,
                job_id,
    ...
    Ok(ListingEvent::ListingsExtracted {
        source_id,
        job_id,
...
async fn handle_extract_listings_from_resource_link(
    ...
    Ok(ListingEvent::ResourceLinkListingsExtracted {
        job_id,
        url,
        listings: extracted_listings,
        context,
