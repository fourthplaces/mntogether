...
impl Effect<ListingCommand, ServerDeps> for CrawlerEffect {
    ...
    async fn execute(
        ...
        match cmd {
            ...
            ListingCommand::RegeneratePagePosts {
                page_snapshot_id,
                job_id,
                requested_by,
                is_admin,
...
async fn generate_missing_embeddings_for_website(
    ...
    let listings = Listing::find_without_embeddings_for_website(
        website_id,
        100,
        &ctx.deps().db_pool,
    )
    ...
    for listing in &listings {
        ...
        match ctx.deps().embedding_service.generate(&content_for_embedding).await {
            Ok(embedding) => {
                if let Err(e) = Listing::update_embedding(listing.id, &embedding, &ctx.deps().db_pool).await {
                    warn!(
...
async fn handle_regenerate_page_summaries(
    ...
    Ok(ListingEvent::PageSummariesRegenerated {
        website_id,
        job_id,
...
async fn handle_regenerate_single_page_summary(
    ...
    let page_snapshot = match PageSnapshot::find_by_id(&ctx.deps().db_pool, page_snapshot_id).await
    {
        Ok(s) => s,
        Err(_) => {
            return Ok(ListingEvent::PageSummaryRegenerated {
                page_snapshot_id,
                job_id,
            });
        }
    ...
    Ok(ListingEvent::PageSummaryRegenerated {
        page_snapshot_id,
        job_id,
    })
}
...
async fn handle_regenerate_single_page_posts(
    ...
    let page_snapshot = match PageSnapshot::find_by_id(&ctx.deps().db_pool, page_snapshot_id).await
    {
        ...
        Err(e) => {
            ...
            return Ok(ListingEvent::PagePostsRegenerated {
                page_snapshot_id,
                job_id,
                posts_count: 0,
            });
    ...
    let listings: Vec<crate::common::ExtractedListing> = extracted_listings
        .into_iter()
        .map(|listing| crate::common::ExtractedListing {
            title: listing.title,
            tldr: listing.tldr,
            description: listing.description,
            contact: listing.contact.map(|c| crate::common::ContactInfo {
                phone: c.phone,
                email: c.email,
                website: c.website,
            }),
    ...
    Ok(ListingEvent::PagePostsRegenerated {
        page_snapshot_id,
        job_id,
        posts_count,
    })
