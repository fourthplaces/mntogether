...
import {
  useGraphQL,
  graphqlMutateClient,
  invalidateAllMatchingQuery,
} from "@/lib/graphql/client";
import { AdminLoader } from "@/components/admin/AdminLoader";
import { GET_SYNC_BATCHES, GET_SYNC_PROPOSALS } from "@/lib/graphql/queries";
import {
  APPROVE_SYNC_PROPOSAL,
...
function BatchProposals({
  ...
  }>(GET_SYNC_PROPOSALS, { batchId }, { revalidateOnFocus: false });

  const proposals = data?.syncProposals || [];
  ...
  const handleAction = async (
    ...
    try {
      const mutation =
        action === "approve" ? APPROVE_SYNC_PROPOSAL : REJECT_SYNC_PROPOSAL;
      await graphqlMutateClient(mutation, { proposalId });
      invalidateAllMatchingQuery(GET_SYNC_PROPOSALS);
      invalidateAllMatchingQuery(GET_SYNC_BATCHES);
    } catch (err) {
      alert(
...
function BatchCard({ batch }: { batch: SyncBatch }) {
  ...
  const pendingCount =
    batch.proposalCount - batch.approvedCount - batch.rejectedCount;
  const hasPending =
    pendingCount > 0 &&
  ...
  const handleBatchAction = async (action: "approve" | "reject") => {
    ...
    try {
      const mutation =
        action === "approve" ? APPROVE_SYNC_BATCH : REJECT_SYNC_BATCH;
      await graphqlMutateClient(mutation, { batchId: batch.id });
      invalidateAllMatchingQuery(GET_SYNC_BATCHES);
      invalidateAllMatchingQuery(GET_SYNC_PROPOSALS);
    } catch (err) {
      alert(
...
export default function ProposalsPage() {
  ...
  }>(GET_SYNC_BATCHES, variables, { revalidateOnFocus: false });

  const batches = data?.syncBatches || [];
    ...
    <div className="min-h-screen bg-stone-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-white border border-stone-200 rounded-lg p-4">
            <div className="text-2xl font-bold text-stone-900">
              {batches.reduce((sum, b) => sum + b.proposalCount, 0)}
            </div>
