...
export function Chatroom({ isOpen, onClose, withAgent = "admin" }: ChatroomProps) {
  ...
  const { data: recentChatsData, isLoading: loadingRecent } = useGraphQL<GetRecentChatsResult>(
    GET_RECENT_CHATS,
    { limit: 1 },
  ...
  const handleStartNewChat = useCallback(async () => {
    setIsCreating(true);
    try {
      const data = await graphqlMutateClient<CreateChatResult>(CREATE_CHAT, {
        language: "en",
  ...
  }, [isOpen, recentChatsData, loadingRecent, autoStarted, isCreating, handleStartNewChat]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      await graphqlMutateClient<SendMessageResult>(SEND_MESSAGE, {
        containerId,
                ...
                >
                  ...
                  <p
                    className={`text-xs mt-1 ${
                      message.role === "user" ? "text-amber-200" : "text-stone-400"
                    }`}
                  >
