...
export function LinksBElement(props: PlateElementProps) {
  ...
  return (
    <PlateElement {...rest} element={element} editor={editor} className="links-b" contentEditable={false}>
      {links.map((link, i) => (
        <div key={i} className="links-b__item" style={{ display: "flex", gap: "8px", padding: "8px 0" }}>
          ...
          <button type="button" className="block-action-btn" onClick={() => updateLinks(links.filter((_, j) => j !== i))}>×</button>
        </div>
      ))}
      <div className="block-actions">
        <button type="button" className="block-action-btn" onClick={() => updateLinks([...links, { title: "", url: "" }])}>+ Link</button>
      </div>
      {children}
    </PlateElement>
  );
