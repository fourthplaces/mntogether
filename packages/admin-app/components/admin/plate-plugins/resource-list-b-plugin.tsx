      ...
      <ul className="list-b" style={{ listStyle: "none", padding: 0 }}>
        {items.map((item, i) => (
          <li key={i} style={{ display: "flex", gap: "8px", alignItems: "baseline", borderBottom: "1px dotted rgba(0,0,0,0.1)", padding: "8px 0" }}>
            ...
            <button type="button" className="block-action-btn" onClick={() => updateItems(items.filter((_, j) => j !== i))}>×</button>
          </li>
        ))}
      </ul>
      <div className="block-actions">
        <button type="button" className="block-action-btn" onClick={() => updateItems([...items, { name: "", detail: "" }])}>+ Item</button>
      </div>
      {children}
    </PlateElement>
  );
