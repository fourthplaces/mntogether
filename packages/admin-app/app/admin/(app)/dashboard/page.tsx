...
function StatewideCallout({
  ...
  const copy: Record<
    Bucket,
    { title: string; tone: string; href: string; cta: string }
  > = {
    missing: {
      title: "Statewide edition: not generated for this week",
      tone: "bg-amber-50 border-amber-200",
      href: "/admin/editions?status=draft",
      cta: "Generate & set up",
    },
    draft: {
      title: "Statewide edition: draft",
      tone: "bg-yellow-50 border-yellow-200",
      href: editionId ? `/admin/editions/${editionId}` : "/admin/editions",
      cta: "Open draft",
    },
    in_review: {
      title: "Statewide edition: in review",
      tone: "bg-amber-50 border-amber-200",
      href: editionId ? `/admin/editions/${editionId}` : "/admin/editions",
      cta: "Continue review",
    },
    approved: {
      title: "Statewide edition: approved, ready to publish",
      tone: "bg-emerald-50 border-emerald-200",
      href: editionId ? `/admin/editions/${editionId}` : "/admin/editions",
      cta: "Open",
    },
    published: {
      title: "Statewide edition: published ✓",
      tone: "bg-green-50 border-green-200",
      href: editionId ? `/admin/editions/${editionId}` : "/admin/editions",
      cta: "View",
    },
  ...
  return (
    <div
      className={`rounded-lg border ${c.tone} px-4 py-3 mb-4 flex items-center justify-between gap-4`}
    >
      <span className="text-sm font-medium text-foreground">{c.title}</span>
      <Button size="sm" variant="outline" render={<Link href={c.href} />}>
        {c.cta}
      </Button>
    </div>
  );
