...
export function Home() {
  ...
  const postCounts = useMemo(() => {
    ...
    posts.forEach((post) => {
      const type = post.postType as PostTypeFilter;
          ...
          <>
            ...
            <div className="mb-6 flex items-center justify-between">
              <p className="text-sm text-gray-500">
                Showing <span className="font-medium text-gray-900">{filteredPosts.length}</span> resource{filteredPosts.length !== 1 ? 's' : ''}
                {searchQuery && (
