...
export function PostCard({ post }: PostCardProps) {
  ...
  const getUrgencyStyles = (urgency?: string) => {
    switch (urgency) {
      ...
      default:
        return { bg: 'bg-white', border: 'border-gray-200', text: 'text-gray-700', badge: 'bg-gray-100 text-gray-700' };
    }
  };

  const getPostTypeStyles = (postType?: string) => {
    switch (postType) {
      case 'service':
        return { bg: 'bg-blue-100', text: 'text-blue-700', icon: '🏥', label: 'Service' };
      case 'opportunity':
        return { bg: 'bg-emerald-100', text: 'text-emerald-700', icon: '🤝', label: 'Opportunity' };
      case 'business':
        return { bg: 'bg-purple-100', text: 'text-purple-700', icon: '🏪', label: 'Business' };
      case 'professional':
        return { bg: 'bg-indigo-100', text: 'text-indigo-700', icon: '👤', label: 'Professional' };
      default:
        return { bg: 'bg-gray-100', text: 'text-gray-700', icon: '📋', label: 'Resource' };
    }
  };

  const getCapacityStyles = (status?: string) => {
    switch (status) {
      case 'accepting':
  ...
  return (
    <div className={`rounded-xl border ${urgencyStyles.border} ${urgencyStyles.bg} p-5 hover:shadow-lg transition-all duration-200 flex flex-col h-full`}>
      {/* Header: Post Type + Urgency */}
      <div className="flex items-center justify-between mb-3">
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${postTypeStyles.bg} ${postTypeStyles.text}`}>
          <span>{postTypeStyles.icon}</span>
          {postTypeStyles.label}
        </span>
        <div className="flex items-center gap-2">
          {capacityStyles && (
            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${capacityStyles.bg} ${capacityStyles.text}`}>
