                ...
                <div className="space-y-2">
                  {post.contacts.map((c) => (
                    <div key={c.id} className="text-sm text-[#4D4D4D]">
                      {c.contactLabel && <span className="text-xs text-[#A09A8D] block">{c.contactLabel}</span>}
                      {c.contactType === "website" || c.contactType === "booking_url" || c.contactType === "social" ? (
                        <a href={c.contactValue.startsWith("http") ? c.contactValue : `https://${c.contactValue}`} target="_blank" rel="noopener noreferrer" className="text-[#8B6D3F] hover:text-[#6D5530] underline break-all">{c.contactValue}</a>
                      ) : c.contactType === "email" ? (
                        <a href={`mailto:${c.contactValue}`} className="text-[#8B6D3F] hover:text-[#6D5530] underline">{c.contactValue}</a>
                      ) : c.contactType === "phone" ? (
                        <a href={`tel:${c.contactValue}`} className="text-[#8B6D3F] hover:text-[#6D5530] underline">{c.contactValue}</a>
                      ) : (
